...
function displayMovie(movies) {
  ...
  movies.forEach((movie) => {
    (kod += `<div class="card m-1" style="width: 18rem;">
                            <img src="${
                              movie.Poster
                            }" class="card-img-top" alt="no photo">
                            <div class="card-body">
                            ...
                            <ul class="list-group list-group-flush">
                                ${field
                                  ...
                                  .reduce(
                                    (k, f) =>
                                      (k += `<li class="list-group-item"><b>${f}: </b>${movie[f]}</li>`),
                                    ""
                                  )}
                                <button class="btn btn-primary m-2" data-bs-toggle="modal" data-bs-target="#modal-${
                                  movie.imdbID
                                }">Show Details</button>
                            </ul>
                        </div>`),
      (kod += `<div class="modal fade" id="modal-${
        movie.imdbID
                        ...
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="modal-${
                                  movie.imdbID
                                }-label">${movie.Title}</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                                <img src="${
                                  movie.Poster
                                }" class="img-fluid mb-3" alt="no photo">
                                <ul>
                                    ${field.reduce(
                                      (k, f) =>
                                        (k += `<li><b>${f}: </b>${movie[f]}</li>`),
                                      ""
                                    )}
                                </ul>
                            </div>
...
function displayPagination() {
  ...
  for (let i = 1; i <= totalPages; i++) {
    kod += `<button class="btn btn-dark mx-1 my-2" onclick="goToPage(${i})">${i}</button>`;
  }
